...
t_sum <- data.frame("Level" = c("Two Baseline Assessments", "Gain on at least one Baseline 2 Composite Score", "Gain on all Baseline 2 Composite Scores"), "Total" = c(18739, 18556, 1130))
...
time_data_baseline_3 <- all_double_baseline %>% 
  select(passport_id, test_date_1, test_date_2, time_between) %>% 
  mutate(time_between = as.numeric(time_between))
...
```{r}
time_react(time_data_baseline_3, time_between)
```

## Plot 3
```{r}
time_hist(time_data_baseline_3, time_between)
```
...
```{r, include=FALSE}
head(all_double_baseline)

mean_2(all_double_baseline$time_between)

mod_baselines <- all_double_baseline %>% 
  mutate(time_between = as.numeric(time_between))

mean_2(mod_baselines$time_between)

mod_baselines2 <- mod_baselines %>% 
  select(passport_id,
         time_between,
         memory_verbal_diff,
         memory_visual_diff,
         impulse_control_diff,
         reaction_time_diff,
         visual_motor_diff)

mod_baselines2_long <- mod_baselines2 %>% 
  pivot_longer(
    cols = c(3:7),
    names_to = "composite",
...
#diff = baseline 2 - baseline 1

mod <- lm(memory_verbal_diff ~ time_between, mod_baselines)
summary(mod)

mod2 <- lm(memory_visual_diff ~ time_between, mod_baselines)
summary(mod2)

mod3 <- lm(impulse_control_diff ~ time_between, mod_baselines)
summary(mod3)

mod4 <- lm(reaction_time_diff ~ time_between, mod_baselines)
summary(mod4)

mod5 <- lm(visual_motor_diff ~ time_between, mod_baselines)
summary(mod5)

# 4 and 5 may be significant

ggplot(mod_baselines, aes(memory_verbal_diff, time_between)) +
  ...
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.x = element_line(color = "gray80")) +
  labs(x = "Memory Verbal Score Difference",
       y = "Number of Days")

sc_plt <- function(df, x, y) {
  ggplot(df, aes({{x}}, {{y}})) +
  geom_smooth(method = "lm",
              se = FALSE) +
  geom_point(color = "gray40",
...
sc_plt(mod_baselines, memory_verbal_diff, time_between)
```
...
### Scatterplot
```{r}
sc_plt(mod_baselines, memory_visual_diff, time_between)
```
...
### Scatterplot
```{r}
sc_plt(mod_baselines, impulse_control_diff, time_between)
```
...
### Scatterplot
```{r}
sc_plt(mod_baselines, reaction_time_diff, time_between)
```
...
### Scatterplot
```{r}
sc_plt(mod_baselines, visual_motor_diff, time_between)
```
...
pi_all <- pi_all %>% 
  mutate(mem_verbal_diff = user_memory_composite_score_verbal_baseline - user_memory_composite_score_verbal_post_injury_1,
         mem_visual_diff = user_memory_composite_score_visual_baseline - user_memory_composite_score_visual_post_injury_1,
         impulse_control_diff = user_impulse_control_composite_score_baseline - user_impulse_control_composite_score_post_injury_1,
         reaction_time_diff = user_reaction_time_composite_score_baseline - user_reaction_time_composite_score_post_injury_1,
         visual_motor_diff = user_visual_motor_composite_score_baseline - user_visual_motor_composite_score_post_injury_1)


pi_mem_verbal_t <- pi_mem_verbal %>% 
  mutate(mem_verbal_diff = user_memory_composite_score_verbal_baseline - user_memory_composite_score_verbal_post_injury_1)

pi_mem_verbal_all_long <- pi_mem_verbal_t %>% 
  select(passport_id, test_date_baseline, test_date_post_injury_1, user_memory_composite_score_verbal_baseline, user_memory_composite_score_verbal_post_injury_1, mem_verbal_diff)  %>% 
  pivot_longer(
    cols = c(4:5),
    names_to = "test_time",
...
ggplot(pi_mem_verbal_all_long, aes(test_time, score)) +
  geom_boxplot() +
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.x = element_line(color = "gray80")) +
...
box_p(pi_mem_verbal_all_long, test_time, score)



pi_mem_verbal_baseline <- pi_mem_verbal_t$user_memory_composite_score_verbal_baseline
pi_mem_verbal_pi <- pi_mem_verbal_t$user_memory_composite_score_verbal_post_injury_1

pi_mem_verbal_res <- t.test(pi_mem_verbal_baseline, pi_mem_verbal_pi, paired = TRUE)

pi_mem_verbal_res

mean_2(pi_mem_verbal_t$user_memory_composite_score_verbal_baseline)
mean_2(pi_mem_verbal_t$user_memory_composite_score_verbal_post_injury_1)
```
...
```{r, include=TRUE}
create_react(pi_mem_verbal_t, user_memory_composite_score_verbal_baseline)
```

### Baseline Total Score Histogram
```{r, include=TRUE}
score_hist(pi_mem_verbal_t, user_memory_composite_score_verbal_baseline)
```

### Post-Injury Total Descriptive Statistics
```{r, include=TRUE}
create_react(pi_mem_verbal_t, user_memory_composite_score_verbal_post_injury_1)
```

### Baseline 2 Total Score Histogram
```{r, include=TRUE}
score_hist(pi_mem_verbal_t, user_memory_composite_score_verbal_post_injury_1)
```
...
```{r, include=FALSE}
pi_mem_visual_t <- pi_mem_visual %>% 
  mutate(mem_visual_diff = user_memory_composite_score_visual_baseline - user_memory_composite_score_visual_post_injury_1)

pi_mem_visual_all_long <- pi_mem_visual_t %>% 
  select(passport_id, test_date_baseline, test_date_post_injury_1, user_memory_composite_score_visual_baseline, user_memory_composite_score_visual_post_injury_1, mem_visual_diff)  %>% 
  pivot_longer(
    cols = c(4:5),
    names_to = "test_time",
...
pi_mem_visual_baseline <- pi_mem_visual_t$user_memory_composite_score_visual_baseline
pi_mem_visual_pi <- pi_mem_visual_t$user_memory_composite_score_visual_post_injury_1

pi_mem_visual_res <- t.test(pi_mem_visual_baseline, pi_mem_visual_pi, paired = TRUE)

pi_mem_visual_res

mean_2(pi_mem_visual_t$user_memory_composite_score_visual_baseline)
mean_2(pi_mem_visual_t$user_memory_composite_score_visual_post_injury_1)
```

### Boxplot
```{r, include=TRUE}
box_p(pi_mem_visual_all_long, test_time, score)
```
...
### Baseline Total Descriptive Statistics
```{r, include=TRUE}
create_react(pi_mem_visual_t, user_memory_composite_score_visual_baseline)
```

### Baseline Total Score Histogram
```{r, include=TRUE}
score_hist(pi_mem_visual_t, user_memory_composite_score_visual_baseline)
```

### Post-Injury Total Descriptive Statistics
```{r, include=TRUE}
create_react(pi_mem_visual_t, user_memory_composite_score_visual_post_injury_1)
```

### Post-Injury Total Score Histogram
```{r, include=TRUE}
score_hist(pi_mem_visual_t, user_memory_composite_score_visual_post_injury_1)
```
...
```{r, include=FALSE}
pi_impulse_control_t <- pi_impulse_control %>% 
  mutate(impulse_control_diff = user_impulse_control_composite_score_baseline - user_impulse_control_composite_score_post_injury_1)

pi_impulse_control_all_long <- pi_impulse_control_t %>% 
  select(passport_id, test_date_baseline, test_date_post_injury_1, user_impulse_control_composite_score_baseline, user_impulse_control_composite_score_post_injury_1, impulse_control_diff)  %>% 
  pivot_longer(
    cols = c(4:5),
    names_to = "test_time",
...
pi_impulse_control_baseline <- pi_impulse_control_t$user_impulse_control_composite_score_baseline
pi_impulse_control_pi <- pi_impulse_control_t$user_impulse_control_composite_score_post_injury_1

pi_impulse_control_res <- t.test(pi_impulse_control_baseline, pi_impulse_control_pi, paired = TRUE)

pi_impulse_control_res

mean_2(pi_impulse_control_t$user_impulse_control_composite_score_baseline)
mean_2(pi_impulse_control_t$user_impulse_control_composite_score_post_injury_1)
```

### Boxplot
```{r, include=TRUE}
box_p(pi_impulse_control_all_long, test_time, score)
```
...
### Baseline Total Descriptive Statistics
```{r, include=TRUE}
create_react(pi_impulse_control_t, user_impulse_control_composite_score_baseline)
```

### Baseline Total Score Histogram
```{r, include=TRUE}
score_hist(pi_impulse_control_t, user_impulse_control_composite_score_baseline)
```

### Post-Injury Total Descriptive Statistics
```{r, include=TRUE}
create_react(pi_impulse_control_t, user_impulse_control_composite_score_post_injury_1)
```

### Post-Injury Total Score Histogram
```{r, include=TRUE}
score_hist(pi_impulse_control_t, user_impulse_control_composite_score_post_injury_1)
```
...
```{r, include=FALSE}
pi_reaction_time_t <- pi_reaction_time %>% 
  mutate(reaction_time_diff = user_reaction_time_composite_score_baseline - user_reaction_time_composite_score_post_injury_1)

pi_reaction_time_all_long <- pi_reaction_time_t %>% 
  select(passport_id, test_date_baseline, test_date_post_injury_1, user_reaction_time_composite_score_baseline, user_reaction_time_composite_score_post_injury_1, reaction_time_diff)  %>% 
  pivot_longer(
    cols = c(4:5),
    names_to = "test_time",
...
pi_reaction_time_baseline <- pi_reaction_time_t$user_reaction_time_composite_score_baseline
pi_reaction_time_pi <- pi_reaction_time_t$user_reaction_time_composite_score_post_injury_1

pi_reaction_time_res <- t.test(pi_reaction_time_baseline, pi_reaction_time_pi, paired = TRUE)

pi_reaction_time_res

mean_2(pi_reaction_time_t$user_reaction_time_composite_score_baseline)
mean_2(pi_reaction_time_t$user_reaction_time_composite_score_post_injury_1)
```

### Boxplot
```{r, include=TRUE}
box_p(pi_reaction_time_all_long, test_time, score)
```
...
### Baseline Total Descriptive Statistics
```{r, include=TRUE}
create_react(pi_reaction_time_t, user_reaction_time_composite_score_baseline)
```

### Baseline Total Score Histogram
```{r, include=TRUE}
score_hist(pi_reaction_time_t, user_reaction_time_composite_score_baseline)
```

### Post-Injury Total Descriptive Statistics
```{r, include=TRUE}
create_react(pi_reaction_time_t, user_reaction_time_composite_score_post_injury_1)
```

### Post-Injury Total Score Histogram
```{r, include=TRUE}
score_hist(pi_reaction_time_t, user_reaction_time_composite_score_post_injury_1)
```
...
```{r, include=FALSE}
pi_visual_motor_t <- pi_visual_motor %>% 
  mutate(visual_motor_diff = user_visual_motor_composite_score_baseline - user_visual_motor_composite_score_post_injury_1)

pi_visual_motor_all_long <- pi_visual_motor_t %>% 
  select(passport_id, test_date_baseline, test_date_post_injury_1, user_visual_motor_composite_score_baseline, user_visual_motor_composite_score_post_injury_1, visual_motor_diff)  %>% 
  pivot_longer(
    cols = c(4:5),
    names_to = "test_time",
...
pi_visual_motor_baseline <- pi_visual_motor_t$user_visual_motor_composite_score_baseline
pi_visual_motor_pi <- pi_visual_motor_t$user_visual_motor_composite_score_post_injury_1

pi_visual_motor_res <- t.test(pi_visual_motor_baseline, pi_visual_motor_pi, paired = TRUE)

pi_visual_motor_res

mean_2(pi_visual_motor_t$user_visual_motor_composite_score_baseline)
mean_2(pi_visual_motor_t$user_visual_motor_composite_score_post_injury_1)
```

### Boxplot
```{r, include=TRUE}
box_p(pi_visual_motor_all_long, test_time, score)
```
...
### Baseline Total Descriptive Statistics
```{r, include=TRUE}
create_react(pi_visual_motor_t, user_visual_motor_composite_score_baseline)
```
...
### Post-Injury Total Descriptive Statistics
```{r, include=TRUE}
create_react(pi_visual_motor_t, user_visual_motor_composite_score_post_injury_1)
```

### Post-Injury Total Score Histogram
```{r, include=TRUE}
score_hist(pi_visual_motor_t, user_visual_motor_composite_score_post_injury_1)
```
...
ggplot(all_double_baseline, aes(user_memory_composite_score_verbal_1, user_memory_composite_score_visual_1)) +
  geom_point() +
  geom_smooth(method = "lm")

cor_prac <- cor.test(all_double_baseline$user_memory_composite_score_verbal_1,
                     all_double_baseline$user_memory_composite_score_visual_1,
                     method = "pearson")
